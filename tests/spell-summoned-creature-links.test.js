...
const detailRendererSource = fs.readFileSync('assets/js/reference-detail-renderer.js', 'utf8');
...
assert.match(detailRendererSource, /function renderSummonedCreatureLink\(spell\)/);
assert.match(detailRendererSource, /spell\?\.(creatura_evocata|creatura_evocata\?\.id)/);
assert.match(detailRendererSource, /spell\?\.creatura_evocata\?\.fonte !== 'mostri'/);
assert.match(detailRendererSource, /appState\.data\.monsters\.find\(\(entry\) => entry\.id === summonedCreatureId\)/);
assert.match(detailRendererSource, /Creatura evocata:/);
assert.match(detailRendererSource, /href="#\/monsters\/\$\{encodeURIComponent\(monster\.id\)\}"/);
assert.match(detailRendererSource, /\$\{renderSummonedCreatureLink\(spell\)\}/);
