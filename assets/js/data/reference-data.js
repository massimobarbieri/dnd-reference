import {
  ...
  normalizeMagicItem,
  normalizeMonster,
} from './normalizers.js?v=20260520-guided';

import {
  fetchJson,
  fetchText,
  parseMonsterImages,
} from './loaders.js?v=20260520-guided';
...
export async function loadReferenceSources(paths) {
  const [monsters, spells, classes, species, backgrounds, equipment, feats, languages, magicItems, rules, rulesGlossary, monsterImageYaml] = await Promise.all([
    ...
    paths.equipment ? fetchJson(paths.equipment) : Promise.resolve([]),
    paths.feats ? fetchJson(paths.feats) : Promise.resolve([]),
    paths.languages ? fetchJson(paths.languages) : Promise.resolve([]),
  ...
  return { monsters, spells, classes, species, backgrounds, equipment, feats, languages, magicItems, rules, rulesGlossary, monsterImageYaml };
}
...
export function applyReferenceSources(appState, sources) {
  const { monsters, spells, classes, species, backgrounds, equipment, feats, languages, magicItems, rules, rulesGlossary, monsterImageYaml } = sources;
  ...
  appState.data.equipment = normalizeArray(equipment);
  appState.data.feats = normalizeArray(feats);
  appState.data.languages = normalizeArray(languages);
